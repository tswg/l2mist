...
public class ItemAuctionInstance
{
		...
		public void runImpl() throws Exception
		{
			...
			switch(state)
			{
				case CREATED:
				{
					...
					{
						String[] params = {};
						Announcements.getInstance().announceByCustomMessage("core.gameserver.model.instances.L2ItemAuctionBrokerInstance.announce." + _auction.getInstanceId(), params);
					}
					checkAndSetCurrentAndNextAuction();
					break;
				}
	...
	void onAuctionFinished(ItemAuction auction)
	{
		...
		if(bid != null)
		{
			...
			{
				Map<Integer, Long> _item = new FastMap<>();
				_item.put(item.getItemId(), 1L);
				Functions.sendSystemMail(player, "Auction item", "Auction", _item);
