...
public class DDSConverter
{
	...
	public static ByteBuffer convertToDDS(InputStream input) throws IOException
	{
		if(input == null)
		{
	...
	private static ByteBuffer convertToDxt1NoTransparency(BufferedImage image)
	{
		if(image == null)
		{
			return null;
	...
	private static ByteBuffer convertToDxt3(BufferedImage image)
	{
		if(image == null)
	...
	private static void buildHeaderDxt1(ByteBuffer buffer, int width, int height)
	{
		buffer.rewind();
		buffer.put((byte) 'D');
		buffer.put((byte) 'D');
	...
	private static void buildHeaderDxt3(ByteBuffer buffer, int width, int height)
	{
		buffer.rewind();
		buffer.put((byte) 'D');
		buffer.put((byte) 'D');
	...
	private static int[] determineExtremeColors(Color[] colors)
	{
	...
	private static long computeBitMask(Color[] colors, int[] extremaIndices)
	{
	...
	private static Color getColor565(int pixel)
	{
	...
	private static Color[] getColors888(int[] pixels)
	{
	...
	private static int distance(Color ca, Color cb)
	{
