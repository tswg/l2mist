...
public class Formulas
{
	...
	public static boolean calcBlow(Creature activeChar, Creature target, Skill skill)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("core.gameserver.skills.Formulas.Chance", (Player)activeChar, new Object[0]).addString(skill.getName()).addNumber((long) chance));
		}
	...
	{
		long reuseDelay;
		if(skill.isUseMacro() && actor.isPlayer() && actor.getPlayer().isUseMacro() && skill.getReuseDelay() < 9000)
			reuseDelay = skill.getReuseDelay() - 3000;
		else
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		if(caster.isPlayer() && (Config.SKILLS_CHANCE_SHOW || caster.getPlayer().isGM()))
		{
			...
			{
				player.sendMessage(new CustomMessage("core.gameserver.skills.Formulas.Chance", player, new Object[0]).addString(skill.getName()).addNumber(Math.round(env.value)));
			}
		}
		if(caster.isMonster())
			env.value *= Config.SKILLS_MOB_CHANCE;
