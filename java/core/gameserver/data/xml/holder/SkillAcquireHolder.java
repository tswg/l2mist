...
public final class SkillAcquireHolder extends AbstractHolder
{
		...
		switch(type)
		{
			case NORMAL:
				if(Config.ALT_SKILL_LEARN)
					skills = _normalSkillTree.get(player.getSkillLearningClassId().getId());
				else
	...
	public Collection<SkillLearn> getAvailableSkills(Player player, int classId)
	{
	...
	public SkillLearn getSkillLearn(Player player, int id, int level, AcquireType type)
	{
		...
		switch(type)
		{
			...
			case TRANSFER_EVA_SAINTS:
				if(Config.ALT_SKILL_LEARN)
					skills = _transferSkillTree.get(player.getSkillLearningClassId().getId());
				else
