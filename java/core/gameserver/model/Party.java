...
import core.gameserver.network.l2.s2c.ExAskModifyPartyLooting;
import core.gameserver.network.l2.s2c.ExMPCCClose;
import core.gameserver.network.l2.s2c.ExMPCCOpen;
import core.gameserver.network.l2.s2c.ExPartyPetWindowAdd;
import core.gameserver.network.l2.s2c.ExPartyPetWindowDelete;
import core.gameserver.network.l2.s2c.ExSetPartyLooting;
import core.gameserver.network.l2.s2c.GetItem;
import core.gameserver.network.l2.s2c.L2GameServerPacket;
import core.gameserver.network.l2.s2c.PartyMemberPosition;
import core.gameserver.network.l2.s2c.PartySmallWindowAdd;
import core.gameserver.network.l2.s2c.PartySmallWindowAll;
import core.gameserver.network.l2.s2c.PartySmallWindowDelete;
import core.gameserver.network.l2.s2c.PartySmallWindowDeleteAll;
...
public class Party implements PlayerGroup
{
	...
	public boolean addPartyMember(Player player)
	{
		...
		if(leader.getVar("AllowInstanceAll") != null)
			if(leader.getVar("AllowInstanceAll").equals("3"))
			{
				leader.unsetVar("AllowInstanceAll");
				if(leader.getActiveReflection() != null)
					leader.getActiveReflection().collapse();
			}
		if(player.getVar("AllowInstanceAll") != null)
			if(player.getVar("AllowInstanceAll").equals("3"))
			{
				player.unsetVar("AllowInstanceAll");
				if(player.getActiveReflection() != null)
					player.getActiveReflection().collapse();
			}
	...
	public boolean removePartyMember(Player player, boolean kick)
	{
		...
		if(player.getVar("AllowInstanceAll") != null)
			if(player.getVar("AllowInstanceAll").equals("2") || player.getVar("AllowInstanceAll").equals("1"))
			{
				player.unsetVar("AllowInstanceAll");
				if(isLeader && player.getParty().getCommandChannel() == null)
					if(player.getActiveReflection() != null)
						player.getActiveReflection().collapse();
			}
