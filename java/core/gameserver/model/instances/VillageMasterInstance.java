...
import core.gameserver.model.base.ClassType;
import core.gameserver.model.base.PlayerClass;
...
import core.gameserver.model.entity.residence.Dominion;
...
import core.gameserver.network.l2.s2c.PledgeReceiveSubPledgeCreated;
...
import core.gameserver.network.l2.s2c.PledgeStatusChanged;
...
import core.gameserver.utils.CertificationFunctions;
...
{
	private static final long serialVersionUID = 6546144691792200497L;
		...
		else if(command.startsWith("Subclass"))
		{
			...
			switch(intVal)
			{
				case 1: // Возвращает список сабов, которые можно взять (см case 4)
					...
					{
						player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", player));
						return;
					}
					break;
				...
				case 4: // Добавление сабкласса - обработка выбора из case 1
					...
					{
						player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", player).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
						allowAddition = false;
					}
					 ...
					 */
					if(!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS && !playerClassList.isEmpty())
						if(Config.RVRMODE_ENABLE && player.isRvRClassHave() && playerClassList.size() < 3 + Config.ALT_GAME_SUB_ADD)
						{
							if(player.isQuestCompleted("_234_FatesWhisper"))
							{
								if(player.getRace() == Race.kamael)
								{
									...
									if(!allowAddition)
										player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.QuestSeedsOfChaos", player));
								}
								else
								{
									allowAddition = player.isQuestCompleted("_235_MimirsElixir");
									if(!allowAddition)
										player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.QuestMimirsElixir", player));
								}
							}
							else
							{
								player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.QuestFatesWhisper", player));
								allowAddition = false;
							}
						}
						else if(playerClassList.size() < 2 + Config.ALT_GAME_SUB_ADD)
						{
							if(player.isQuestCompleted("_234_FatesWhisper"))
							{
								if(player.getRace() == Race.kamael)
					...
					if(allowAddition)
					{
						...
						{
							player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", player));
							return;
						}
				...
				case 5: // Смена саба на другой из уже взятых - обработка выбора из case 2
					...
					player.setActiveSubClass(classId, true, true);
				...
				case 7: // Отмена сабкласса - обработка выбора из case 6
					...
					{
						if(classId == 97)
							player.getInventory().destroyItemByItemId(15307, 1L);
						
						if(classId == 105)
							player.getInventory().destroyItemByItemId(15308, 1L);
						
						if(classId == 112)
						{
							List<ItemInstance> HolyPomanders = player.getInventory().getItemsByItemId(15309);
							for(ItemInstance i : HolyPomanders)
								player.getInventory().destroyItem(i, 1L);
						}
	...
	public void setLeader(Player leader, String newLeader)
	{
		...
		if(member == null)
		{
			...
			//leader.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.S1IsNotMemberOfTheClan", leader).addString(newLeader));
		...
		{
			leader.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.CannotAssignUnitLeader", leader));
			return;
		}
	...
	{
		player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.ClanLeaderWillBeChangedFromS1ToS2", player).addString(clan.getLeaderName()).addString(newLeader.getName()));
	...
	public void createSubPledge(Player player, String clanName, int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if(pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if(subLeader == null)
			{
				player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader", player));
				return;
			}
			else if(subLeader.getLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader", player));
				return;
			}
		}
	...
	public void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if(clan == null)
		{
			player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.ClanDoesntExist", player));
			return;
		}
		...
		if(targetUnit == null)
		{
			player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.SubUnitNotFound", player));
			return;
		}
		SubUnit mainUnit = clan.getSubUnit(Clan.SUBUNIT_MAIN_CLAN);
		...
		player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2VillageMasterInstance.NewSubUnitLeaderHasBeenAssigned", player));
	}
	...
	private Set<PlayerClass> getAvailableSubClasses(Player player, boolean isNew)
	{
		...
		PlayerClass currClass = PlayerClass.values()[baseClassId];// .valueOf(charClassName);
