...
import core.gameserver.model.MinionList;
import core.gameserver.model.Player;
import core.gameserver.model.Skill;
import core.gameserver.model.SkillLearn;
import core.gameserver.model.Spawner;
import core.gameserver.model.TeleportLocation;
import core.gameserver.model.Territory;
import core.gameserver.model.World;
import core.gameserver.model.Zone.ZoneType;
import core.gameserver.model.actor.listener.NpcListenerList;
import core.gameserver.model.actor.recorder.NpcStatsChangeRecorder;
...
import core.gameserver.model.entity.events.objects.TerritoryWardObject;
...
import core.gameserver.network.l2.s2c.AcquireSkillDone;
import core.gameserver.network.l2.s2c.AcquireSkillList;
import core.gameserver.network.l2.s2c.AutoAttackStart;
import core.gameserver.network.l2.s2c.ExChangeNpcState;
import core.gameserver.network.l2.s2c.ExShowBaseAttributeCancelWindow;
import core.gameserver.network.l2.s2c.ExShowVariationCancelWindow;
import core.gameserver.network.l2.s2c.ExShowVariationMakeWindow;
...
import core.gameserver.taskmanager.DecayTaskManager;
...
import core.gameserver.templates.npc.Faction;
import core.gameserver.templates.npc.NpcTemplate;
import core.gameserver.templates.spawn.SpawnRange;
...
{
	private static final long serialVersionUID = 887874287294895873L;
	...
	public void setEnchantEffect(int val)
	{
		_enchantEffect = val;
	}
							...
							while ((count == 0) && (classCheck != null))
							{
								for (ClassId cid : _classesToTeach)
								{
									if (cid.level() != classCheck.level())
										continue;
									if (SkillAcquireHolder.getInstance().getAvailableSkills(player, cid.getId()).isEmpty())
										continue;
									text += "<a action=\"bypass -h npc_%objectId%_SkillList " + cid.getId() + "\">Learn " + cid + "'s class Skills</a><br>\n";
									count++;
								}
								classCheck = classCheck.getParent(player.getSex());
							}
						}
						else
							text += "No Skills.<br>\n";
	...
	public void showSkillList(Player player)
	{
		...
		if(getTemplate().getTeachInfo().isEmpty() && !Config.ALT_SKILL_LEARN)
		{
			NpcHtmlMessage html = new NpcHtmlMessage(player, this);
			StringBuilder sb = new StringBuilder();
		...
		if(!Config.ALT_SKILL_LEARN)
		{
			if(!(getTemplate().canTeach(classId)|| getTemplate().canTeach(classId.getParent(player.getSex()))))
			{
				...
				else
				{
					...
					StringBuilder sb = new StringBuilder();
					sb.append("<html><head><body>");
					sb.append(new CustomMessage("core.gameserver.model.instances.L2NpcInstance.WrongTeacherClass", player));
					sb.append("</body></html>");
					html.setHtml(sb.toString());
					player.sendPacket(html);
				}
		...
		for(SkillLearn s : skills)
		{
			...
			if(sk == null || !sk.getCanLearn(player.getClassId()) && !Config.ALT_SKILL_LEARN || !sk.canTeachBy(npcId) && !Config.ALT_SKILL_LEARN)
				continue;

			counts++;
			if(Config.ALT_SKILL_LEARN_MAX_LVL)
				asl.addSkill(s.getId(), SkillTable.getInstance().getMaxLevel(s.getId()), SkillTable.getInstance().getMaxLevel(s.getId()), s.getCost(), 0);
			else
	...
	public void showSkillList(Player player, ClassId classId)
	{
        ...
        if (!(getTemplate().canTeach(classId) || getTemplate().canTeach(classId.getParent(player.getSex()))))
        {
            ...
            TextBuilder sb = new TextBuilder();
            sb.append("<html><head><body>");
            sb.append(new CustomMessage("l2dream.gameserver.model.instances.L2NpcInstance.WrongTeacherClass", player));
            sb.append("</body></html>");
            html.setHtml(sb.toString());
            player.sendPacket(html);
            return;
        ...
        int counts = 0;
        final Collection<SkillLearn> skills = SkillAcquireHolder.getInstance().getAvailableSkills(player, classId.getId());
		AcquireSkillList asl = new AcquireSkillList(AcquireType.NORMAL, skills.size());
        for (SkillLearn s : skills)
        {
            Skill sk;
			if(Config.ALT_SKILL_LEARN_MAX_LVL)
				sk = SkillTable.getInstance().getInfo(s.getId(), SkillTable.getInstance().getMaxLevel(s.getId()));
			else
	...
	public void setNpcRace(Race race)
	{
		_NpcRace = race;
	}
}