...
public class GameObjectTasks
{
	...
	public static class halfCastTimeLaunchedTask extends RunnableImpl
	{
		public boolean _forceUse;
		boolean isOneTargetOffensive = false;
		
		private final HardReference<? extends Creature> _charRef;

		public halfCastTimeLaunchedTask(Creature cha, boolean forceUse)
		{
			_charRef = cha.getRef();
			_forceUse = forceUse;
		}
		...
		public void runImpl()
		{
			...
			if(castingSkill.isOffensive() && castingSkill.oneTarget())
				isOneTargetOffensive = true;
			if(!GeoEngine.canSeeTarget(character, castingTarget, false) && character.isCastingNow() && isOneTargetOffensive)
			{
				character.abortCast(true, true);
				if(character.isPlayable())
					character.getPlayer().sendPacket(SystemMsg.CANNOT_SEE_TARGET);
			}
		}
	}
