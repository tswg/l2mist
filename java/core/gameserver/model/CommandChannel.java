...
import core.gameserver.model.instances.NpcFriendInstance;
...
import core.gameserver.network.l2.s2c.ExMPCCPartyInfoUpdate;
...
public class CommandChannel implements PlayerGroup
{
	...
	public CommandChannel(Player leader)
	{
		...
		for(Player member : leader.getParty())
			if(member.getVar("AllowInstanceAll") != null)
				if(member.getVar("AllowInstanceAll").equals("2"))
					member.unsetVar("AllowInstanceAll");
	...
	public void removeParty(Party party)
	{
		...
		for(Player player : party.getPartyMembers())
		{
			if(player.getVar("AllowInstanceAll") != null)
				if(player.getVar("AllowInstanceAll").equals("1"))
				{
					player.unsetVar("AllowInstanceAll");
					if(player == _commandChannelLeader)
