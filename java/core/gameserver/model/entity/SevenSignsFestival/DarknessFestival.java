package core.gameserver.model.entity.SevenSignsFestival;
	...
	public void collapse()
	{
		...
		if(SevenSigns.getInstance().getCurrentPeriod() == SevenSigns.PERIOD_COMPETITION && getParty() != null)
		{
			...
			if(player.getInventory().destroyItem(bloodOfferings))
			{
				...
				sendCustomMessageToParticipants("core.gameserver.model.entity.SevenSignsFestival.Ended");
				if(isHighestScore)
					sendMessageToParticipants("Your score is highest!");
			}
			else
				player.sendMessage(new CustomMessage("core.gameserver.model.instances.L2FestivalGuideInstance.BloodOfferings", player));
		}
