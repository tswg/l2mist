...
public class BookMarkList
{
	...
	public boolean tryTeleport(int slot)
	{
		...
		{
			if(!RvRMode.FrendlyTown(owner, new Location(bookmark.x, bookmark.y, bookmark.z)))
			{
				 owner.sendMessage(owner.isLangRus() ? "Эта местность захвачена вражеской расой!" : "This location is captured by enemy race!");
				 return false;
			}
		}
	...
	public static boolean checkTeleportLocation(Player player, int x, int y, int z)
	{
		...
		if(Config.RVRMODE_ENABLE)
		{
			if(!RvRMode.FrendlyTown(player, new Location(x, y, z)))
			{
				 player.sendMessage(player.isLangRus() ? "Эта местность захвачена вражеской расой!" : "This location is captured by enemy race!");
				 return false;
			}
		}
